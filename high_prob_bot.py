...
class AdaptiveParams:
    ...
    def choose_pattern(self) -> str:
        if np.random.random() < 0.1:
            return np.random.choice(list(self.pattern_weights.keys()))
...
class AdaptiveOrderManager:
    ...
    def can_enter(self, current_price) -> bool:
        ...
        if now - self.last_trade_time < self.cooldown_seconds:
            return False
        # Minimum price movement check (avoid trading in stale price)
    ...
    def execute_sell(self, price: float, contracts: int = 1):
        ...
        self.entry_price = 0.0
        self.entry_pattern = None
        # Do NOT reset last_trade_time here – cooldown applies to next entry
        # We keep last_trade_time as the time of the last BUY, not SELL.
    ...
    try:
        while True:
            ...
            if orders.position == 0:
                # Only check for new entry if cooldown and price movement allow
                if orders.can_enter(price):
                    signal = strategy.get_signal(price)
                    if signal == "BUY":
                        orders.execute_buy(price, strategy.last_pattern_used, CONTRACTS)
                else:
                    # Log occasionally that we are in cooldown
                    if int(time.time()) % 60 < SCAN_SECONDS:
                        log.info(f"Cooldown active or price stale – no entry. BTC: ${price:,.2f}")
            else:
                # We are in a position – check if we should exit
                # For simplicity, exit after 30 seconds (you can add trailing stop later)
